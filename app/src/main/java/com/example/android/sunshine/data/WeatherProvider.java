...
//      DONE (9) Handle queries on both the weather and weather with date URI
        switch (sUriMatcher.match(uri)) {
            case CODE_WEATHER:
                returnedCursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, selection,
                        selectionArgs, null, null, sortOrder);
                break;
            case CODE_WEATHER_WITH_DATE:
                String normalizedUtcDateString = uri.getLastPathSegment();
                String[] selectionArguments = new String[] {normalizedUtcDateString};
                returnedCursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME, projection,
